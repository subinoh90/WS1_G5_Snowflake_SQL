...
SELECT DISTINCT
  ...
  ROUND(SUM(total_account_balance)
        OVER (PARTITION BY TRIM(customer_id)), 2) AS total_assets,
  ROUND(SUM(total_liabilities)
        OVER (PARTITION BY TRIM(customer_id)), 2) AS total_liabilities,
  INITCAP(LOWER(TRIM(preferred_channel))) AS preferred_channel,
  TO_CHAR(date_of_birth,'YYYYMMDD') AS date_of_birth_key,
  INITCAP(LOWER(TRIM(city))) AS city,
  INITCAP(LOWER(TRIM(gender))) AS gender,
  ROUND(AVG(income)
        OVER (PARTITION BY TRIM(customer_id)), 2) AS income,
  TO_CHAR(join_date,'YYYYMMDD') AS join_date_key,
  INITCAP(LOWER(TRIM(marital_status))) AS marital_status
FROM silver.customer_products;
